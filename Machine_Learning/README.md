...
5. To find how well the split of `dosage<15` works, we calculate gain given by $Gain=LeftLeaveSimilarity + RightLeaveSimilarity-RootSimilarity$
...
8. Pruning in XG boost is based on the gain value (even the $\lambda$ does pruning to an extent). We declare $\gamma$ with a some value say 130. If the lowest branch in our tree is less that 130, then we remove that branch and move to the upper branch. We start pruning from lowest branch not from root!
